...
void SimpleModel::loadModel(ModelType type, bool texture)
{
	const char* filename = getFilenameFromModelType(type);
	// Create an instance of the Importer class
	Assimp::Importer importer;

	// load model file with assimp 
	const aiScene *scene = importer.ReadFile(getModelFile(type).c_str(),
...
const char* SimpleModel::getFilenameFromModelType(ModelType type)
{
	switch (type)
	{
	case ModelType::Cube:
